...
- **The slide distance is set in code, not in the resource.** An animator resource can only carry a
  ...
  `SettingsFragment` overrides it — main's `hold` carries no distance and runs as declared.
...
- `androidx.fragment` is pinned to 1.9.0 in the version catalog even though nothing in the app calls
  it directly. Navigation 2.10.0 still asks for fragment 1.6.2, and seekable fragment effects arrived
  in 1.7.0 — drop the dependency and predictive back degrades in silence to the non-interactive pop.
